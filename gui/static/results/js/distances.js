...
var DISTANCES = (function(){
	...
	var create_tab = function(data){
		...
		if( centers_path !== "" ){
			// Add the tab
			$("<li><a href='#centers-tab'>Selection Centers</a></li>").insertAfter("#clusters_tab");

			// handle data
			data.cluster_centers = JSON.parse(COMM.synchronous.load_external_text_resource(centers_path));

			$("#distances-tab").append($(render()));
